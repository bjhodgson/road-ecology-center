...
# Set paths
excel_file <- "C:\\Users\\HP\\Downloads\\Untreated Points Medians (2).xlsx"
...
freq_table <- table(medians_df$MedianType)
print(freq_table)

# Assuming 'MedianType' in 'medians_df' contains roadkill data
roadkill_counts <- table(medians_df$MedianType)

# Assuming 'MednTyp' in 'cleaned_gdf' contains random points data
random_counts <- table(cleaned_gdf$MednTyp)

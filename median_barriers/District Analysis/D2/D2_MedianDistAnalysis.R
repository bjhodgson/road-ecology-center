...
script_dir <- "C://Users//HP//Documents//GitHub//road-ecology-center//median_barriers//District Analysis//D2" # Set path to folder containing RScripts
...
# WVC_gdf <- WVC_gdf %>% # TOO FEW CASES, DISRUPTS CHI-SQUARE TEST
...
#   mutate(MedianType = case_when( # TOO FEW CASES, DISRUPTS CHI-SQUARE TEST
#     MedianType == "vegetative" & grepl("grass", SecondaryAttribute) ~ paste0(MedianType, " (grass)"),
#     MedianType == "vegetative" & grepl("sparse shrubs/trees", SecondaryAttribute) ~ paste0(MedianType, " (sparse shrubs/trees)"),
#     MedianType == "vegetative" & grepl("dense trees", SecondaryAttribute) ~ paste0(MedianType, " (dense shrubs/trees)"),
...
random_counts <- merged_random_gdf %>%
  ...
  mutate(MedianType = case_when( # Standardize values
    MedianType == "vegetative, thrie beam" ~ "thrie beam", # Change to thrie beam, otherwise count too low
    MedianType == "thrie beam, vegetative" ~ "thrie beam", # Change to thrie beam, otherwise count too low
    MedianType == "gravel, thrie beam" ~ "thrie beam",
    MedianType == "gravel, vegetative" ~ "vegetative",
    MedianType == "vegetative, gravel" ~ "vegetative",
    MedianType == "temp concrete" ~ "concrete",
    TRUE ~ MedianType  # Retain original value if no condition is met
  )) %>%
  #filter(!MedianType %in% c("cable", "gravel")) %>% # Remove expected counts under 5 to validate chi-square test
  count(MedianType) %>%
  rename(Count = n)  # Rename the count column to "Count"
print(sum(random_counts$Count))

contingency_table <- full_join(WVC_counts, random_counts, by = "MedianType") %>%
  rename(
    WVC = Count.x,
    Random = Count.y
  ) %>%
  replace_na(list(WVC = 0, Random = 0))
...
total_observed <- sum(contingency_table$WVC)
total_expected <- sum(contingency_table$Random)

# Calculate proportions in the Random group
observed_proportions <- WVC_counts$Count / total_observed
expected_proportions <- contingency_table$Random / total_expected

# Perform Chi-Square Goodness of Fit Test
chisq_test <- chisq.test(contingency_table$WVC, p = expected_proportions)
print(chisq_test)

# Write contingency table w/ proportions to csv
contingency_table <- contingency_table %>%
  cbind(observed_proportions, expected_proportions)
#write.csv(contingency_table, file.path(file_dir, "D2_squirrel_hits_chisq.csv"), row.names = FALSE)

# Fisher's Exact Test

# Create the contingency table
contingency_matrix <- as.matrix(contingency_table[, c("WVC", "Random")])
...
# Perform Chi-Square Goodness of Fit Test w/ Monte Carlo
chisq_test <- chisq.test(contingency_table$WVC, p = expected_proportions, simulate.p.value = TRUE, B = 2000)
print(chisq_test)

Mode(merged_random_gdf$MedianWidth, na.rm=TRUE)


fisher_test_result <- fisher.test(as.matrix(contingency_table))
print(fisher_test_result)

# K-S test -- not appropriate for discrete data

ks.test(contingency_table$WVC, contingency_table$Random)
...
# Define the median types
median_types <- c("concrete", "no median", "vegetative", "cable", "gravel", "thrie beam")

# Generate random counts for WVC (observed) and Random (expected)
# Adjust the ranges as needed to simulate different scenarios
WVC_random <- sample(0:100, length(median_types), replace = TRUE)   # Random counts for WVC
Random_random <- sample(0:100, length(median_types), replace = TRUE) # Random counts for expected values

# Create the contingency table
contingency_table_random <- data.frame(
  MedianType = median_types,
  WVC = WVC_random,
  Random = Random_random
)

# Display the generated contingency table
print(contingency_table_random)

# Calculate total counts
total_observed <- sum(contingency_table_random$WVC)
total_expected <- sum(contingency_table_random$Random)

# Calculate proportions in the Random group
expected_proportions <- contingency_table_random$Random / total_expected


# Perform Chi-Square Goodness of Fit Test w/ Monte Carlo
chisq_test <- chisq.test(contingency_table_random$WVC, p = expected_proportions, simulate.p.value = TRUE, B = 2000)
print(chisq_test)

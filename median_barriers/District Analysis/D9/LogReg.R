...
script_dir <- "C:\\Users\\HP\\Documents\\GitHub\\road-ecology-center\\median_barriers\\District Analysis\\D9"
...
WVC_binary <- WVC_gdf %>% # WVC df
  as.data.frame() %>%
  filter(animal == "Mule (or Black tailed) Deer") %>% # Filter by deer WVCs
...
binary_df <- rbind(WVC_binary, random_binary) %>%
  mutate(MedianType = case_when( # Standardize values
    ...
    MedianType == "vegetative, cable" ~ "cable",
...
exp(coef(summary(model))["AADT", "Estimate"])
...
WVC_binary <- WVC_binary %>%
  mutate(Collision_Outcome = ifelse(condition == "Dead", 1, 0))

model <- glm(Collision_Outcome ~ AADT, 
             data = WVC_binary, 
             family = binomial)
model

coef(summary(model))["Collision_Outcome", "Estimate"]
exp(coef(summary(model))["Collision_Outcome", "Estimate"])

summary(model)$coefficients["Collision_Outcome", "Pr(>|z|)"]
